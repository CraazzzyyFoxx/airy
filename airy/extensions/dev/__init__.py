...
async def run_sql(ctx: AiryPrefixContext) -> None:
    ...
    async with in_transaction("default") as tconn:
        return_value = await tconn.execute_query(sql)
...
async def blacklist_cmd(ctx: AiryPrefixContext, mode: str, user: hikari.User) -> None:
    ...
    entry = BlacklistModel.filter(entry_id=user.id).first()
    ...
    if mode.casefold() == "add":
        ...
        model = await BlacklistModel.create(entry_id=user.id)
        ...
        await BlacklistModel.filter(entry_id=user.id).delete()
        await ctx.event.message.add_reaction("✅")
...
async def resetsettings_cmd(ctx: AiryPrefixContext, guild_id: int) -> None:
    ...
    await GuildModel.filter(guild_id=guild_id).delete()
